...
double collision_circles_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  for (auto wp : spiral) {
    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      // Circle placement:
      // Ref: Udacity Motion Planning Boundary Conditions Offset Goals Section
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // Ref: Udacity Motion Planning Circle Based Collision Detection Section
          auto delta_center_x = (circle_center_x - actor_center_x);
          auto delta_center_y = (circle_center_y - actor_center_y);
          double dist = std::sqrt(((delta_center_x*delta_center_x) + (delta_center_y*delta_center_y)));
